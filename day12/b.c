...
void traverse_map(map m, set *s, char *node, char *path, int *twice, int *ended)
{
    ...
    for (int i = 0; i < m.keys[id].count; i++)
    {
        ...
        if (!strcmp(next_node, "start") || (set_contains(node_set, next_node) && (nt++)))
            continue;
        ...
        traverse_map(m, &node_set, next_node, path_to, &nt, ended);
    }
    free(path_to);
}
...
int main(void)
{
    ...
    traverse_map(cave, &small_caves, "start", "", &tw, &ended);
    printf("%d\n", ended);
